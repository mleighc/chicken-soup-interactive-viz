...
base = (
    ...
    .encode(x=alt.X(alt.repeat("repeat"), type="quantitative", bin=alt.Bin(maxbins=20)), y="count()")
    .properties(width=190, height=160)
)
...
chart1 = alt.layer(background, highlight, data=chicken_soups).repeat(
    repeat=["Calories_kcal", "Fat_g", "Carbohydrates_g", "Cholesterol_mg", "Sodium_mg", "Protein_g"],
    columns=3,
)
# chart1
...
colorCondition = alt.condition(
    selection, alt.Color("healthScore:Q", scale=alt.Scale(scheme="reds")), alt.value("lightgrey")
)
...
base = (
    alt.Chart()
    .mark_bar(color="lightgreen")
    .encode(x=alt.X(alt.repeat("column"), type="quantitative", bin=alt.Bin(maxbins=20)), y="count()")
    .properties(width=190, height=160)
)
...
# layer the two charts & repeat
chart_3 = alt.layer(background, highlight, data=chicken_soups).repeat(
    column=["fat_per_serving", "calories_per_serving", "vit_D_per_serving"]
)
...
section_options = [
    ...
    "Inspiration",
    "Evaluation",
    "Chart 1: Nutrient Facts Distributions",
    "Chart 2: Dairy Content and Nutrition",
    "Chart 3: Flu-Fighting Nutrients",
    "Chart 4: Recipes By Serving Size",
]

with st.sidebar:
    chart_options_select = st.selectbox(label="Select a section to view: ", options=section_options)
...
elif chart_options_select == "Chart 1: Nutrient Facts Distributions":
...
elif chart_options_select == "Chart 2: Dairy Content and Nutrition":
...
elif chart_options_select == "Chart 3: Flu-Fighting Nutrients":
...
elif chart_options_select == "Chart 4: Recipes By Serving Size":
